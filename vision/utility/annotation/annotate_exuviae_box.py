...
# === CONFIG ===
CSV_PATH = "../../data/inat_raw/inat_dataset.csv"   # CSV file
IMAGE_ROOT = "../../data/inat_raw"                  # images under <stage>/<obs>_<photo>.jpg
...
# Filter: target stages & missing exuviae box
mask = (
    df["stage"].isin(TARGET_STAGES) &
    df[["x_exuviae","y_exuviae","w_exuviae","h_exuviae"]].isna().any(axis=1)
)
df_indices = df[mask].index.tolist()
print(f"Annotating {len(df_indices)} EXUVIAE boxes.")
...
while i < len(df_indices):
    ...
    fig, ax = plt.subplots()
    ax.imshow(img)
    ax.set_title(f"{stage.upper()} | {fname} \n (n skip • b back • d delete • q quit)", fontsize=10)

    # Show organism box (blue, solid) if present
    if pd.notna(row["x_organism"]) and pd.notna(row["y_organism"]) and pd.notna(row["w_organism"]) and pd.notna(row["h_organism"]):
        ax.add_patch(Rectangle(
            ...
            linewidth=1.6, edgecolor="b", facecolor="none"
        ))

    # If exuviae already existed (e.g., revisiting), draw dashed red
    if pd.notna(row["x_exuviae"]) and pd.notna(row["y_exuviae"]) and pd.notna(row["w_exuviae"]) and pd.notna(row["h_exuviae"]):
        ax.add_patch(Rectangle(
            ...
            linewidth=1.6, edgecolor="r", facecolor="none", linestyle="--"
        ))
    ...
    def on_press(event):
        ...
        ax.add_patch(current_rect[0]); fig.canvas.draw()

    def on_motion(event):
        if current_rect[0] is not None and event.xdata is not None and event.ydata is not None:
            x0, y0 = rect_coords[0]; x1, y1 = event.xdata, event.ydata
            current_rect[0].set_bounds(min(x0,x1), min(y0,y1), abs(x1-x0), abs(y1-y0))
            fig.canvas.draw_idle()

    def on_release(event):
        if not rect_coords or any(flags.values()) or event.xdata is None or event.ydata is None:
            return
        x0, y0 = rect_coords[0]; x1, y1 = event.xdata, event.ydata
        ...
        print(f"{fname} → exuviae: ({x_min}, {y_min}, {w}, {h})")
        plt.close()
    ...
    def on_key(event):
        if event.key == "n":
            ...
            flags["delete"] = True
            for c in ["x_exuviae","y_exuviae","w_exuviae","h_exuviae"]:
                df.at[idx, c] = pd.NA
            print(f"🗑️ Deleted exuviae box for {fname}")
            plt.close()
        elif event.key == "q":
